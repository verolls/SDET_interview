...
**不可重复读**：事务开启之后，对某数据进行两次读取，发现两次读取的结果不同，也就是说没有读到相同的内容。这是因为有其他事务对这个数据同时进行了增加、修改或删除操作。
> 举例：事务A读取一个数据，事务B对该数据做了修改并提交事务。如果事务A再次读取这个数据，此时读取的结果和第一次读取的结果不同。 

**幻读**：事务在开启之后，读取到的数据永远是刚开启事务时的数据。
> 举例：事务A开启之后，根据条件查询得到N条数据，事务B开启并增加了 M 条符合事务A查询条件的数据，然后事务B提交，当事务A再次根据条件进行查询，依然查询到N条数据。
