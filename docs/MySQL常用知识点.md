...
在创建表的时候，我们可以给表中的字段加上一些约束，约束的作用就是为了保证表中的数据有效。
...
- 主键约束 primary key 约束的字段不能为 NULL，不能重复。
- 被添加主键约束的字段称为主键字段，主键字段中的每一个值称为主键值，主键值是每一行记录的唯一标识。
- 任何一张表都应该有主键。
- 一张表中，只能添加一个主键约束。
- 主键值建议使用 int, bigint, 等定长数值类型。

外键约束：foreign key（简称FK）

- 外键约束用来在两个表数据之间建立链接，其中一张表的一个字段被另一张表中对应的字段约束。也就是说，设置外键约束至少要有两种表，被约束的表叫做从表（子表），另一张叫做主表（父表），属于主从关系。
- 被添加外键约束的字段称为外键字段，外键字段中的每一个值称为外键值。
...
## 数据库并发事务时，可能会产生哪些异常？(你知道哪些并发控制异常？)

数据库并发会带来脏读、不可重复读、幻读这三个常见问题，其中：

**脏读**：事务之间不隔离，读到了其他事务还没有提交的数据，且其他事务进行了回滚，导致读到的数据无效。
>举例：事务A首先读取var变量为50，接着更新为100，但并未提交，事务B在事务A更新var变量后读取var为100，然后事务A回滚。最终事务B读取的var和数据库的var不一样。

**不可重复读**：事务开启之后，对某条数据进行两次读取，发现两次读取的结果不同，也就是说没有读到相同的内容。这是因为有其他事务对这个数据同时进行了增加、修改或删除操作。主要是针对update语句。
> 举例：事务A读取一个数据，事务B对该数据做了修改并提交事务。如果事务A再次读取这个数据，此时读取的结果和第一次读取的结果不同。 

**幻读**：事务A根据条件查询得到了 N 条数据，但此时事务B更改或者增加了 M 条符合事务A查询条件的数据并提交，这样当事务A再次进行查询的时候发现会有 N+M 条数据，产生了幻读，主要是针对insert和delete语句。

## 不可重复读和幻读区别是什么？可以举个例子吗？

**不可重复读针对的对象是单条数据，重点在于update；幻读针对的对象是若干条数据，重点在于insert和delete。**

- 不可重复读（同样的条件, 读取过的数据, 再次读取出来发现值不一样了 ）：事务A第一次读取某员工的工资为1000，事务B修改此员工的工资为2000并提交，事务A第二次读此员工的工资时工资变为 2000；这就是不可重复读。

- 幻读（同样的条件, 第1次和第2次读出来的记录数不一样 ）：某工资单表中工资大于3000的有4人，事务A读取了所有工资大于3000的人，共查到4条记录，这时事务B又插入了一条工资大于3000的记录并提交，事务A再次读取时查到的记录就变为了5条，这就是幻读。


## 如何解决事务并发产生的异常？(你知道哪些事务隔离级别？)

当数据库出现有多个事务同时执行时，就可能出现脏读，不可重复读，幻读等异常，为了解决这些异常，定义了四种事务隔离级别。

- 读未提交(RU, read uncommitted): 一个事务还未提交时，它做的变更就可以被别的事务看到。
>会出现脏读、不可重复读、幻读。相当于事务不隔离，一般不会使用这种隔离级别。

- 读已提交(RC, read committed): 事务提交以后，它做的变更才能被其它事务看到。但是在这个事务未提交之前，数据库中发生的变更，这个事务也能看见。
>不会出现脏读，会出现不可重复读和幻读，Oracle默认级别

- 可重复读(RR, repeatable read): 指一个事务执行过程中看到的数据，一直跟这个事务启动时看到的数据是一致的。
