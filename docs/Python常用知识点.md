...
- 强类型语言：Python中，每个对象都有数据类型，且只支持该类型支持的操作。
- 动态类型语言：Python在运行期间才去做数据类型检查的语言，在用动态类型的语言编程时，永远不用给任何变量指定数据类型。
...
Python中，变量更像是指针，而不是数据存储区域，变量用来指向任意的对象。给变量赋值的操作，简单来说就是将对象的地址传递给变量，或者说，让变量指向对象。
...
## Python的数据类型中，哪些是不可变对象？哪些是可变对象？有什么区别？
...
## 直接赋值、浅拷贝和深拷贝有什么区别？
在对不可变对象进行直接赋值、浅拷贝、深拷贝给新变量的操作时，都是将新变量直接指向不可变对象。

直接赋值、浅拷贝和深拷贝对可变对象会产生不同效果，下面对可变对象进行讨论：
- 直接赋值：将新变量指向可变对象，故前后变量均未隔离，任一对象改变，则所有指向了同一可变对象的变量都作相同改变。  
- 浅拷贝：使用 copy() 函数实现，拷贝可变对象的最外层对象即原对象并实现隔离，但原对象的子对象仍是未被隔离的引用关系。
- 深拷贝：使用 deepcopy() 函数实现，拷贝可变对象的原对象及子对象而非引用，实现对前后变量不论深浅层的完全隔离。
...
## Python中如何使用列表推导式创建一个全零的3乘3的二维数组
```bash
