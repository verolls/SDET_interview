...
## 什么是重写？什么是重载？
重载和重写都是java多态的表现。
- 重载叫override，在同一个类中多态的表现。重载是发生在同一个类中，具有相同的方法名，但是有不同的参数，参数的个数不一样、参数的位置不一样，方法的返回值不一样，这就叫重载，常见的就比如构造方法，有有参构造和无参构造。
- 重写叫overwrite，是字符类中多态的表现。当子类继承父类时，对父类中的一些方法根据自己的需求进行重写操作。
